import { FecthCreateConselingSchedule, FecthHealthCheckup } from "@/services/HealthProfileService";
import { MedicalHealthCheckupRecord } from "@/types/MedicalRecord";
import { useCallback, useEffect, useState } from "react";
import { ChevronDownIcon, ChevronUpIcon, CalendarIcon, ChatBubbleLeftRightIcon, MagnifyingGlassIcon } from "@heroicons/react/24/outline";
...
export default function ManagerRecord() {
    const [healthCheckup, setHealthCheckup] = useState<MedicalHealthCheckupRecord[]>([]);
    const [filteredRecords, setFilteredRecords] = useState<MedicalHealthCheckupRecord[]>([]);
    ...
    const handleSubmit = useCallback(async (e: React.FormEvent) => {
        e.preventDefault();
        if (!formData.studentId || !formData.healthCheckupId || !formData.requestDate) {
    ...
    const handleOpenConsultForm = useCallback((item: MedicalHealthCheckupRecord) => {
        setFormData({
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-blue-50 p-4 rounded-xl border border-blue-100 flex items-center gap-3 mb-6">
                    ...
                    <p className="text-sm text-blue-700">
                        Nhấn vào biểu tượng <CalendarIcon className="w-4 h-4 inline-block mx-1" /> để đặt lịch tư vấn
                    </p>
                </div>
                        ...
                        >
                            ...
                            >
                                <div className="flex justify-between items-start">
                                    <div>
                                        <h2 className="text-lg font-semibold text-gray-800">{item.studentName}</h2>
                            ...
                            {openItemIndex === index && (
                                <div className="border-t border-gray-100 p-4 space-y-4">
                                    <div className="grid grid-cols-2 gap-4">
                                        ...
                                        <div className="bg-gray-50 p-3 rounded-lg">
                                            <Label className="block text-sm font-medium text-gray-700 mb-1">Răng miệng</Label>
                                    ...
                                    <div className="bg-gray-50 p-3 rounded-lg">
                                        <Label className="block text-sm font-medium text-gray-700 mb-1">Ghi chú bất thường</Label>
                                        <p className="text-gray-900 whitespace-pre-wrap">{item.abnormalNote}</p>
                                    </div>
                                </div>
                            )}
