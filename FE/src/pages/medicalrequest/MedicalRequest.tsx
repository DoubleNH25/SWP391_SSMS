...
import { FecthParents, FecthStudentsByParentId, FecthStudents } from "@/services/UserService";
...
export default function MedicalRequest() {
    ...
    const [studentsLoadingForSearch, setStudentsLoadingForSearch] = useState(false);
    ...
    const handleNavigateToStudent = useCallback(() => {
        if (selectedStudentId) {
            navigate(`/medical/create-medical-request/${selectedStudentId}`);
        }
    }, [selectedStudentId, navigate]);

    const handleStudentClick = useCallback((student: Student | null) => {
        if (student) {
            navigate(`/medical/create-medical-request/${student.id}`);
        }
    }, [navigate]);
                            ...
                            <div className="flex gap-2">
                                <div className="flex-1">
                                    <SearchableSelect
                                        ...
                                        placeholder={studentsLoadingForSearch ? "Đang tải..." : "Chọn học sinh theo mã hoặc tên..."}
                                ...
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-colors"
                                >
                                ...
                                {selectedStudentId && (
                                    <>
                                        ...
                                        <span className="font-medium">
                                            {studentOptions.find(opt => opt.value === selectedStudentId)?.label || "N/A"}
                                        </span>
                                    </>
                                )}
                            </span>
