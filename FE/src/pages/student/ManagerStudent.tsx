...
import { PlusIcon, Users, X, Search, EyeIcon, ArrowUp, ArrowDown } from "lucide-react";
...
export default function StudentManager() {
  ...
  const gradeLevels = Array.from(new Set(classes.map(cls => {
    const gradeMatch = cls.className.match(/^(\d+)/);
    return gradeMatch ? gradeMatch[1] : "0";
  }))).filter(grade => grade !== "0").sort();

  // Filter and sort students
  const filteredStudents = students.filter((student) => {
    const matchesClass = !classIdFilter || student.studentClass.id === classIdFilter;
    const matchesSearch = !searchTerm ||
      student.fullName.toLowerCase().includes(searchTerm.toLowerCase()) ||
      student.studentCode.toLowerCase().includes(searchTerm.toLowerCase()) ||
      student.studentClass.className.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    <div className="p-4">
          ...
          {(searchTerm || selectedGrade !== "all" || classIdFilter) && (
            <div className="flex items-center gap-2 text-sm text-gray-600 pt-2">
              ...
              <span>
                ...
                {searchTerm && (selectedGrade !== "all" || classIdFilter) && " và "}
                {selectedGrade !== "all" && <span className="font-medium">khối {selectedGrade}</span>}
                {selectedGrade !== "all" && classIdFilter && " và "}
                {classIdFilter && <span className="font-medium">lớp {getFilteredClassName()}</span>}
              </span>
            </div>
          )}
        </div>
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          >
            <div className="text-center">
              <h2 className="text-lg font-semibold text-gray-900">Import học sinh từ XLSX</h2>
              <p className="mt-2 text-gray-600">Vui lòng chọn file XLSX để import học sinh.</p>
              <input
                type="file"
                      ...
                      <td className="px-6 py-2 whitespace-nowrap text-sm text-gray-500">
                        <div className="flex items-center gap-2">
                          ...
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={() => handleViewHealthCheckupRecords(student.id)}
                            className="hover:bg-blue-500 hover:text-white"
                          >
                            <EyeIcon className="size-4" />
